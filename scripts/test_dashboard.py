...
def test_no_derived_file_is_stale():
    """Every file derived from the career database matches the database.

    This is the check that was missing. After a club-merge round the cards and
    the prerendered pages were regenerated by calling og_cards.py and
    prerender.py directly -- but those are two of the NINE artifacts
    build_dashboard_data.py writes, and the dashboard JSON was not among them.
    The phantom transfers stayed live on the site for a full round with
    transactions.json already clean.

    It compounded: team/*.html is rendered FROM data/team_pages.json, which was
    itself stale, so prerender's byte comparison reported "30 unchanged" while
    comparing against stale input. A green incremental count is only as honest
    as the file it is comparing against.
    """
    import build_dashboard_data as bdd

    players = json.loads(bdd.CAREERS.read_text(encoding="utf-8"))
    stale = bdd.check(players)
    assert not stale, (
        "derived files do not match the career database; run "
        "scripts/build_dashboard_data.py:\n  "
        + "\n  ".join(str(p.relative_to(bdd.ROOT)) for p in stale))
    print(f"test_no_derived_file_is_stale PASS "
          f"({len(bdd.derived_files(players))} files)")


def test_the_dashboard_ledger_agrees_with_the_transaction_log():
    """The widget's rows are the ledger's rows, under the current alias table.

    A club rename that lands in transactions.json but not here is exactly what
    the site showed: a phantom transfer, and a real move labelled with a club
    ...
    n = TeamNormalizer()
    data = json.loads(bdd.OUT.read_text(encoding="utf-8"))
    rows = data.get("latest_signings") or []
    assert rows, "the widget has no rows at all"
    for r in rows:
        a, b = r.get("from_team") or "", r.get("to_team") or ""
        assert n.normalize(a) == a, f"{a!r} is not a canonical club name"
        assert n.normalize(b) == b, f"{b!r} is not a canonical club name"
        assert n.normalize(a) != n.normalize(b), \
            f"phantom transfer on the dashboard: {r}"
    print(f"test_the_dashboard_ledger_agrees_with_the_transaction_log PASS "
          f"({len(rows)} rows)")


if __name__ == "__main__":
    test_no_derived_file_is_stale()
    test_the_dashboard_ledger_agrees_with_the_transaction_log()
